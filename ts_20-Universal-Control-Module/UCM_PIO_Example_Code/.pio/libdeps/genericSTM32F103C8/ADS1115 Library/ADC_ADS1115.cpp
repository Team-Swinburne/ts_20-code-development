 ...
 */
void ADC_ADS1115::setAddr_ADS1115(uint8_t i2cAddress)
{
    ads_i2cAddress = i2cAddress;
    ads_conversionDelay = ADC_ADS1115_CONVERSIONDELAY;
}

/*
 *Sets up the Hardware
 */
void ADC_ADS1115::init()
{
  Wire.begin();
}


bool ADC_ADS1115::checkADS1115()
{
    uint8_t error;
 ...
 */
void ADC_ADS1115::setLowThreshold(int16_t threshold)
{
    ads_lowthreshold = threshold;
    writeAdsReg(ads_i2cAddress, ADC_ADS1115_POINTER_LOWTHRESH, ads_lowthreshold);
}

/*
 *Gets the low threshold value
 */
int16_t ADC_ADS1115::getLowThreshold()
{
 ...
 */
void ADC_ADS1115::setHighThreshold(int16_t threshold)
{
    ads_highthreshold = threshold;
    writeAdsReg(ads_i2cAddress, ADC_ADS1115_POINTER_HITHRESH, ads_highthreshold);
}

/*
 *Gets the high threshold value
 */
int16_t ADC_ADS1115::getHighThreshold()
{
    return ads_highthreshold;
}

void ADC_ADS1115::setCompQue(eADSCompQue_t value){
    int16_t    conf;
    conf = readAdsReg(ads_i2cAddress, ADC_ADS1115_POINTER_CONFIG);
    conf &= ~((uint16_t) 0x03);
    conf |= (uint16_t) value;
    writeAdsReg(ads_i2cAddress, ADC_ADS1115_POINTER_CONFIG, conf);
}

void ADC_ADS1115::setCompLat(eADSCompLat_t value){
    int16_t    conf;
    conf = readAdsReg(ads_i2cAddress, ADC_ADS1115_POINTER_CONFIG);
...
void ADC_ADS1115::setCompPol(eADSCompPol_t value){
    int16_t    conf;
    conf = readAdsReg(ads_i2cAddress, ADC_ADS1115_POINTER_CONFIG);
...
void ADC_ADS1115::setCompMode(eADSCompMode_t value){
    int16_t    conf;
    conf = readAdsReg(ads_i2cAddress, ADC_ADS1115_POINTER_CONFIG);
...
void ADC_ADS1115::setRate(eADSRate_t value){
    int16_t    conf;
    conf = readAdsReg(ads_i2cAddress, ADC_ADS1115_POINTER_CONFIG);
...
void ADC_ADS1115::setMode(eADSMode_t value){
    int16_t    conf;
    conf = readAdsReg(ads_i2cAddress, ADC_ADS1115_POINTER_CONFIG);
...
void ADC_ADS1115::setGain(eADSGain_t value){
    int16_t    conf;
    conf = readAdsReg(ads_i2cAddress, ADC_ADS1115_POINTER_CONFIG);
...
void ADC_ADS1115::setMux(eADSMux_t value){
    int16_t    conf;
    conf = readAdsReg(ads_i2cAddress, ADC_ADS1115_POINTER_CONFIG);
...
void ADC_ADS1115::setOSMode(eADSOSMode_t value){
    int16_t    conf;
    conf = readAdsReg(ads_i2cAddress, ADC_ADS1115_POINTER_CONFIG);
 ...
 */
uint16_t ADC_ADS1115::readVoltage(uint8_t channel)
{
    if (channel > 3)
        return 0;
    ...
    int16_t Voltage = 0;
    Voltage = (int16_t)readAdsReg(ads_i2cAddress, ADC_ADS1115_POINTER_CONVERT)*coefficient;
    return Voltage;
}
/*
 ...
 */
/**************************************************************************/
int16_t ADC_ADS1115::ComparatorVoltage(uint8_t channel)
{
    // Set Comparator mode
    setCompMode(eCOMPMODE_TRAD);
...
void ADC_ADS1115::writeReg(uint8_t i2cAddress, uint8_t reg, uint8_t *pBuf, uint16_t len)
{
    _pWire->begin();
...
void ADC_ADS1115::readReg(uint8_t i2cAddress, uint8_t reg, uint8_t *pBuf, uint16_t len)
{
    _pWire->begin();
    _pWire->beginTransmission(i2cAddress);
    _pWire->write(reg);
...
int16_t ADC_ADS1115::readAdsReg(uint8_t i2cAddress, uint8_t reg)
{
    uint8_t buf[2] = {0};
...
void ADC_ADS1115::writeAdsReg(uint8_t i2cAddress, uint8_t reg, uint16_t value)
{
